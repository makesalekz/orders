...
func (*Order) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case order.FieldID, order.FieldTenantID, order.FieldCounterpartyID, order.FieldCreatedBy:
	...
	for i := range columns {
		...
		case order.FieldCounterpartyID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.CounterpartyID = value.Int64
			}
		case order.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field status", values[i])
			} else if value.Valid {
				_m.Status = enum.OrderStatus(value.String)
			}
